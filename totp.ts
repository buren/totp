 ...
 * @param {number=3} timeStep - configure the time-step, default is 3 minutes.
 */
const generateCode = (token: Buffer, modifier?: string, timeStep: number = 3): number => {
  const hashAlg = crypto.createHmac("sha1", token);

  return computeOtp(hashAlg, getCurrentTimeStepNumber(timeStep), modifier);
}
 ...
 * @param {number=3} timeStep - configure the time-step, default is 3 minutes.
 */
const validateCode = (code: number, token: Buffer, modifier?: string, timeStep: number = 3): boolean => {
  const timeStepNumber = getCurrentTimeStepNumber(timeStep);
